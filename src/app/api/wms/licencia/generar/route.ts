...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user.rol !== 'admin') {
      return NextResponse.json({ error: 'Solo los administradores pueden generar licencias' }, { status: 403 })
    }
    ...
    const fechaVencimiento = tipo === 'vitalicio' ? null : new Date(now.getTime() + days * 24 * 60 * 60 * 1000)
